...
export const savingAdd = (saving, auth, total) => {
  return async (dispatch) => {
    dispatch({ type: ASYNC_ACTION_START, payload: 'savingAdd' });
    const formData = new FormData();
    const random = customAlphabet(NUM_ALPHABET, 5);
    const savingCode = `${yearMonth()}${dateDay()[0]}-SP${random()}`;
    const date = saving.date.toISOString();
    formData.append('savingCode', savingCode);
    formData.append(
      'savingPrimary',
      saving.savingPrimary !== undefined ? saving.savingPrimary : null
    );
    formData.append(
      'savingSecondary',
      saving.savingSecondary !== undefined ? saving.savingSecondary : null
    );
    formData.append(
      'savingTertier',
      saving.savingTertier !== undefined ? saving.savingTertier : null
    );
    formData.append('date', date);
    formData.append('memberId', saving.id);

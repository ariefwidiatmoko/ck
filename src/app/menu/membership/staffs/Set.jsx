...
import { staffAdd } from './redux/reduxApi';
...
class Set extends Component {
  ...
  onSet = (item) => {
    const { data, auth, staffAdd } = this.props;
    staffAdd(auth, item.code, 'Pengurus', data.total);
  };
  ...
  render() {
    ...
    const itemsFilter = items.filter((item) => item.type !== 'Pengurus')
    return (
        ...
        <div className='modal-card'>
          <header className='modal-card-head is-info'>
            <div className='modal-card-title has-text-info has-text-weight-normal'>
              <i className='fas fa-tasks icon' /> Set Pengurus
            </div>
            <button
              onClick={closeModal}
              className='delete'

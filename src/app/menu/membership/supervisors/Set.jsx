...
import { supervisorAdd } from './redux/reduxApi';
...
class Set extends Component {
  ...
  onSet = (item) => {
    const { data, auth, supervisorAdd } = this.props;
    supervisorAdd(auth, item.code, 'Pengawas', data.total);
  };
  ...
  render() {
    ...
    const itemsFilter = items.filter((item) => item.type !== 'Pengawas')
    return (
        ...
        <div className='modal-card'>
          <header className='modal-card-head is-info'>
            <div className='modal-card-title has-text-info has-text-weight-normal'>
              <i className='fas fa-tasks icon' /> Set Pengawas
            </div>
            <button
              onClick={closeModal}
              className='delete'

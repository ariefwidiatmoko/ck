...
import { journalsIndex } from './redux/reduxApi';
...
const mapState = (state) => {
  ...
  if (auth && auth.authorities.details) {
    aS = auth.authorities.details.subm.filter((i) => i.id === 'jurnal')[0];
  }
  const details = state.details;
  let detail = {};
  if (details) {
    detail = details.filter((i) => i.id === 'jurnal')[0];
  }
  return {
    auth: auth,
    aS: aS,
...
class Index extends Component {
  ...
  componentDidMount = () => {
    const { auth, journalsIndex } = this.props;
    const { itn } = this.state;
    journalsIndex(auth.token, itn, 1, '');
  };
  ...
  handleItemNumber = (number) => {
    ...
    journalsIndex(auth.token, number, 1, st);
  };
  ...
  handlePage = (number) => {
    ...
    journalsIndex(auth.token, itn, cp + number, st);
  };
  ...
  handleSimpleSearch = (st) => {
    ...
    journalsIndex(auth.token, itn, 1, st);
  };
  ...
  onClickRecyclebin = () => {
    ...
    journalsIndex(auth.token, itn, 1, 'deleted');
    this.setState({
      mB: !mB,
    });
  };
  ...
  onEdit = (item) => {
    ...
    openModal('JournalEdit', { data: { item: item, auth: auth } });
  };
                      ...
                      >
                        <ul>
                          <li className='is-active'>
                            <Link to='/akuntansi/jurnal'>{tl}</Link>
                          </li>
                          <li className='is-active'>
                            <Link to='/akuntansi/jurnal'>{st !== 'deleted' ? 'Index' : 'Recycle Bin'}</Link>
                          </li>
                        </ul>
                      </nav>
                    </div>
